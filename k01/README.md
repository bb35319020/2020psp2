...
population variance (estimated)：25.361711
```

## 修正履歴
[comment #20200701]
- このレポートの最初は, 自分の氏名と学籍番号にしてください.
- ソースコードの説明を書いてください.
- 出力結果を 入出力結果 の欄に貼り付けてください. 
...
[comment #20200726]
- 質問の件ですが  
l.36 のループ1周目でゼロ割りが発生しています.  
estvarianceの値は次のループで使用するわけではないのでループ内で計算せず, ループを抜けてから計算すると良いと思います.


[comment #20200727]
- nがint型になったことにより, (n-1)/n の計算がゼロになったりして, 正しい答えが出てきていません.  
7/1の修正指示の最後の項目に書いていますが, int掛けるdoubleを先に計算するなど, 計算順序を工夫して対処しましょう. 

[comment #20200729 sonoda]
- グローバル変数は可能な限り使わないようにしましょう．今回の場合の`n`です．
  - `ave_online`や`var_online`は`n`を引数に持つべきです．引数で`n`を関数に渡せばいいはずです．
- 他は指摘事項はありません．修正したらまた連絡してください．
