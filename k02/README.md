...
l30
l31 標準化させる
その値をp_stdnormで標準正規分布にする  
その値を戻り値としてL1,L2に代入してπ計算していく。
...
## 修正履歴
[comment #20200701]
- このレポートの最初は自分の学籍番号と氏名にしてください.
- ソースコードの説明を書いてください.
- 入出力結果のところに出力結果を貼り付けてください.
- 変数名 `max_val`, `min_val` のところはそれぞれ `L1`, `L2` にした方が良いと思います. 


[comment #20200721]
- レポートの最初というのは, このファイルの3行目のことです. 
- `max_val`, `min_val`は使わなくなったので消しましょう. 
- `MUA`, `sigmaA`などは変数ではなく定数なので `const double` または `#define MUA 170.8` のようにしましょう. 


[comment #20200716]
- l.19 mua=MUA など #defineで定義したものを変数に入れていますが, 変数に入れる必要はないので消しましょう. 