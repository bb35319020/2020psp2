...
## ソースコードの説明
l17~21定義
L23~25sscsnｆで文字を読み取る
L38:r_stdnorm ()でZ変換した値に標準偏差をかけ平均を足すことでｚ変換する前の値にする。
L38:その値をdummyに格納する。
...
[comment #20200721]
- 自分の学生番号, 氏名にしましょう
- ソースコードの説明, 出力結果を書きましょう
- l.17〜l.19 これらはコマンドラインから値を入力させるので, ここで値を代入しないでください
- l.23 `num_dummy` はint型です. 適切な書式指定子にしましょう. 
- `argc` を使って, コマンドライン引数の数(個数)がおかしいときにエラーメッセージを出して終了するようにしましょう. 


[comment #20200728]
- `else{ return 0;}` の部分が不要です. これがあるとコマンドライン引数の数が正しくてもここで終了してしまい, その後の計算が行われません.  
あと, エラーメッセージを出すようにしてください. 